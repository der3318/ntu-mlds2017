...
with tf.Session(config=config) as sess:
    ...
    while step <= epoch:
        random.shuffle(data)
        x_batch = numpy.zeros( (batch_size, n_steps, n_input) )
        y_batch = numpy.zeros( (batch_size, n_steps, n_output) )
        curBatch = 0
        for lineNumber, line in enumerate(data):
            ...
            for i in range(n_steps):
                x_batch[curBatch, i, :n_dimensions] = embeddings[ line[i + delta] ][:]
                y_batch[curBatch, i, :] = embeddings[ line[i + delta + 1] ][:]
            ...
            if curBatch == batch_size:
                ...
                print("\r0%" + progress1[pro] + progress2[pro] + "100% - Epoch: " + str(step) + "/" + str(epoch) + " - Line Number: " + str(lineNumber) + " - Loss(forward): " + str(loss), end = "")
                curBatch = 0
        step += 1
        print("")
    if is_load:
        print("\x1b[0;32;40m<Skipping> Loading Model\x1b[0m")
        latest_checkpoint = tf.train.latest_checkpoint("./forward_model/")
        saver = tf.train.import_meta_graph("forward_model/model.meta")
        saver.restore(sess, latest_checkpoint)
    else:
        print("\x1b[0;32;40m<Optimization Finished> Saving Model\x1b[0m")
        saver.save(sess, "forward_model/model")
...
# Output
    with open(sys.argv[4], "w") as fout:
        for lineNumber, line in enumerate(dataTest):
            ...
            x_batch = numpy.zeros( (1, n_steps, n_input) )
            ...
            for i in range(n_steps):
                x_batch[0, i, :n_dimensions] = embeddings[ line[i + start] ][:]
            y_pred = sess.run(pred, feed_dict={x: x_batch})[target, :]
            sims = numpy.array([ numpy.dot( myNormalize(embeddings[ans]), myNormalize(y_pred) ) for ans in ansTest[lineNumber] ])
            if lineNumber == 1000:
                print("Forward Predicted Embedding of Line No.", lineNumber + 1, ": [", y_pred[0], ", ... ,", y_pred[-1], "]")
                print("Forward Cosine Similarity between Answers:", sims)
            for sim in (target + 1) * sims:
                fout.write(str(sim) + " ")
            fout.write("\n")

print("\x1b[0;32;40m<Done> Forward Output File Available\x1b[0m")
